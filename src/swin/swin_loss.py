...
if __name__ == '__main__':
    ...
    csv_file = 'swin_' + dataset + '_loss' +'_e'+str(epochs)+'.csv'
    results =  [{'Epoch': i} for i in range(epochs)]
    lrs = [5e-3, 5e-4, 5e-5]
    for lr in lrs:
        if dataset == 'cifar':
            model = timm.models.swin_transformer.SwinTransformer(img_size  = 32, num_classes = 10,  embed_dim = 64, depths = (2,8,2), num_heads=(2,4,8), patch_size=4, window_size = 8).to(device)
        elif dataset ==  'stl':
            model = timm.models.swin_transformer.SwinTransformer(img_size  = 96, num_classes = 10,  embed_dim = 64, depths = (2,8,2), num_heads=(2,4,8), patch_size=4, window_size = 12).to(device)
        elif dataset ==  'tiny':
            model = timm.models.swin_transformer.SwinTransformer(img_size  = 64, num_classes = 200,  embed_dim = 64, depths = (2,8,2), num_heads=(2,4,8), patch_size=4, window_size = 8).to(device)

        criterion = nn.CrossEntropyLoss()
        params = model.parameters()
        optimizer = torch.optim.Adam(params, lr=lr, betas=(0.9, 0.999), eps=1e-08, weight_decay=0, amsgrad=False)
        ...
        for e in range(epochs):
            ...
            #for inputs, labels in training_loader:
                ...
                print(epoch_results)
                results[e]['training_loss_' + str(lr)] = epoch_loss
                results[e]['validation_loss_' + str(lr)] = val_epoch_loss
                results[e]['training_acc_' + str(lr)] = epoch_acc
                results[e]['validation_acc_' + str(lr)] = val_epoch_acc
        
    csv_columns = ['Epoch']
    for lr in lrs:
        csv_columns.append('training_' + str(lr))
        csv_columns.append('val_' + str(lr))
